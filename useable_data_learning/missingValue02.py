...
#누락된 값은 모두 0으로 치환하시오.
print(myframe.fillna(0,inplaca=False))
print('\n#inplaca=False이므로 원본은 변동 사항이 없다.')
print(myframe)
#fillna() 메소드를 실행하면 매번 새로운 객체로 반환된다.
#오리지널 기존 객체를 변경하려면 inplaca=True 옵션을 사용하면 된다.
#inplaca=True 옵션 : 복사본을 생성하지 않고, 호출한 객체 자체를 변경하겠다(기본값 : False).
print('\n#inplaca=True이므로 원본 데이터가 변경된다.')
myframe.fillna(0,inplaca=True)
print(myframe)
print('\n#누락된 데이터가 있는 샘플 데이터프레임')
...
myframe.fillna(mydict,inplaca=True)
print(myframe)
